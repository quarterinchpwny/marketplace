<!-- Footer -->

<footer class="ftr text-center text-lg-start text-light pt-0"">

            <!-- Section: Links  -->
            <section class="footerbody">
                <div class="container text-md-start">
                <!-- Grid row -->
                    <div class="row contentsFooter">
                        <!-- Grid column -->

                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-3 col-xl-3">
                             <!-- Content -->
                                <img src="/images/masculineImages/rizalLogo.png" alt="Rizal Logo" id="logoFooter">
                                <p class="text-center">Lorem ipsum dolor sit amet</p>
                            </div>
                            <!-- Grid column -->

                            <!-- Grid column -->
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-3 col-xl-3 mx-auto mb-4 spacing">
                             <!-- Links -->
                                <h6 class="text-left text-uppercase fw-bold mb-4 spacing" id="footerHeading">
                                    Visit
                                </h6>
                                <p class="text-left" id="footerInfo">
                                    Unit 608, 6th floor,<br>
                                    AIC Burgundy Empire Tower
                                    ADB Avenue corner Garnet Road
                                    Ortigas Center, Pasig City
                                </p>
                            </div>
                            <!-- Grid column -->

                            <!-- Grid column -->
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-3 col-xl-3 mb-4 spacing">
                             <!-- Links -->
                                <h6 class="text-left text-uppercase fw-bold mb-4 spacing" id="footerHeading">
                                    Contact
                                </h6>
                                <p class="text-left" id="footerInfo" >
                                    (02) 84511594<br>
                                    contact@rizallawoffice.com
                                </p>
                            </div>
                            <!-- Grid column -->

                            <!-- Grid column -->
                            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-3 col-xl-3 mx-auto mb-md-0 mb-4 pb-3">
                                <!-- Links -->
                                    <h6 class="text-left text-uppercase fw-bold mb-4 spacing" id="footerHeading">
                                        Socials
                                    </h6>
                                <div class="logocenter">
                                    <a href="#" class="me-3 text-reset">
                                        <img src="/images/masculineImages/fbLogo.svg" alt="facebook logo" height=25px width=25px>
                                    </a>
                                    <a href="" class="me-3 text-reset">
                                        <img src="/images/masculineImages/linkedInLogo.svg" alt="LinkedIn logo" height=25px width=25px>
                                    </a>
                                    <a href="" class="me-3 text-reset">
                                        <img src="/images/masculineImages/ytLogo.svg" alt="YouTube logo" height=25px width=25px>
                                    </a>
                                    <a href="" class="me-3 text-reset">
                                        <img src="/images/masculineImages/igLogo.svg" alt="Instagram logo" height=25px width=25px>
                                    </a>
                                    <a href="" class="me-3 text-reset">
                                        <img src="/images/masculineImages/twtLogo.svg" alt="Twitter logo" height=25px width=25px>
                                    </a>
                                </div>
                            </div>

                        <!-- Grid column -->
                    </div>

                <!-- Grid row -->
                </div>



            </section>
<hr>
  <div class="container text-md-start">
        <div class="row">

                <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
                    <p class="text-left text-primary">Powered by <img src="/images/masculineImages/lexmeetLogo.png"  alt="lexlogo" height="13" width="85"></p>
                </div>

                <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
                    <a href="" class="text-left disc">Disclaimer</a></ul>
                </div>

                <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
                     <a href="" class="text-left disc">Terms of Use</a>
                </div>

                <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2">
                    <a href="" class="text-left disc">Privacy Policy</a>
                </div>

                <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 col-xl-3">
                    <a href="" class="text-left disc">Informed Consent of Service Performed</a>
                </div>

        </div>

    </div>

</footer>